...
export class AIActivityProvider implements vscode.WebviewViewProvider {
    ...
    constructor(private readonly _extensionUri: vscode.Uri) {
        ...
        // Set up periodic refresh every 1 second for testing
        setInterval(() => {
            this.refreshPrompts();
        }, 1000);
    }
    ...
    private processSpecStoryContent(content: string, topic: string) {
        ...
        // Find ALL user prompts in the content - much more flexible detection
        const userPrompts = [];
        
        // Debug: log first 15 lines to understand format
        logDebug('=== First 15 lines of content for analysis ===');
        lines.slice(0, 15).forEach((line, index) => {
            logDebug(`Line ${index + 1}: "${line}"`);
        });
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Much broader detection patterns for SpecStory format
            const isUserPrompt = 
                ...
                // SpecStory specific patterns - look for Czech question words and patterns
                (line.length > 10 && 
                 ...
                 !lowerLine.startsWith('---') &&
                 !lowerLine.startsWith('|') &&
                 !lowerLine.startsWith('generated by') &&
                 // Look for question-like patterns and Czech prompts
                 (lowerLine.includes('?') || 
                  lowerLine.includes('můžeš') || 
                  lowerLine.includes('mohl') ||
                  lowerLine.includes('prosím') ||
                  lowerLine.includes('please') ||
                  lowerLine.includes('dej') ||
                  lowerLine.includes('udělej') ||
                  lowerLine.includes('vytvořte') ||
            ...
            if (isUserPrompt) {
                logDebug(`*** FOUND POTENTIAL USER PROMPT at line ${i + 1}: "${line}"`);
                
                // Extract the prompt text - handle various formats
                let promptText = line;
                ...
                // If still no meaningful text, use original line
                ...
                while (nextLineIndex < lines.length && linesCollected < 5) {
                    ...
                    if (nextLower.startsWith('assistant') || 
                        ...
                        nextLine.startsWith('---') ||
                        nextLine.startsWith('<!--') ||
                        nextLine.includes('Generated by SpecStory')) {
                        break;
                    }
                    
                    // Add this line to the prompt if it's meaningful
                ...
                if (fullPrompt.length > 5) {
                    userPrompts.push(fullPrompt);
                    logDebug(`*** CONFIRMED: Added user prompt ${userPrompts.length}: "${fullPrompt.substring(0, 100)}..."`);
                }
            }
        }

        logDebug(`*** FINAL RESULT: Extracted ${userPrompts.length} user prompts from content`);
